  ...
  async register(createUserDto) {
    ...
    if (existingUser) {
      throw new common_1.BadRequestException(
        "Email ou Nom d'utilisateur déjà pris.",
      );
    }
    ...
    const user = this.usersRepository.create({
      ...
      roleGeneral:
        createUserDto.roleGeneral ??
        user_general_roles_enum_1.UserGeneralRoleEnum.Utilisateur,
    });
  ...
  async login(loginDto) {
    ...
    if (!user) {
      throw new common_1.NotFoundException(
        ValidationErrors_1.ValidationErrors.USER_NOT_FOUND,
      );
    }
    ...
    if (!passwordMatch) {
      throw new common_1.UnauthorizedException(
        ValidationErrors_1.ValidationErrors.CREDENTIALS_INVALID,
      );
    }
